...
Fluid.plugins = {
  ...
  initFancyBox: function(selector) {
    ...
    jQuery(selector || '.markdown-body :not(a) > img, .markdown-body > img').each(function() {
      ...
      imageUrl = imageUrl.slice(imageUrl.lastIndexOf('/'))
      const newImg = `.${imageUrl}`
      var $imageWrap = $image.wrap(`
        <a class="fancybox fancybox.image" href="${newImg}"
          itemscope itemtype="http://schema.org/ImageObject" itemprop="url"></a>`
      ).parent('a');
      $imageWrap[0].innerHTML = `<img src=\"${newImg}\" alt=\"${$imageWrap[0].innerText}\"><p class=\"image-caption\">${$imageWrap[0].innerText}\</p>`
      console.log( $imageWrap[0])
